...
def generate_report(client_name, total_score, severity, breakdown, answers):
    ...
    if breakdown["suicidal_ideation_score"] >= 1:
        si_note = (
            f"\n⚠ IMPORTANT: Client endorsed suicidal ideation at level "
            f"{breakdown['suicidal_ideation_score']} on item 9. Address this explicitly."
        )
...
FLAGGED ITEMS (score ≥ 2): {', '.join(breakdown['flagged_items']) if breakdown['flagged_items'] else 'None'}
{si_note}
...
4. ITEM-LEVEL CLINICAL OBSERVATIONS (highlight items scoring ≥ 2; address suicidal ideation separately if endorsed)
5. RISK CONSIDERATIONS
6. CLINICAL FORMULATION
7. TREATMENT RECOMMENDATIONS
8. SUMMARY

Use formal clinical language. Reference actual scores and responses. Ready for placement in a clinical file."""
...
def create_pdf_report(path, client_name, total_score, severity, report_text, answers, timestamp):
    ...
    def sev_color(label):
        m = {"Normal":"#4CAF50","Mild":"#8BC34A","Borderline":"#FFC107",
    ...
    flag_s     = ParagraphStyle("F",  fontName="Helvetica-Bold",fontSize=9.5,textColor=RED,   leading=14)
    ...
    story.append(Paragraph("Beck Depression Inventory", title_s))
    story.append(Paragraph("Clinical Assessment Report", sub_s))
    ...
    t = Table(info_data, colWidths=[3*cm, 6.5*cm, 3*cm, 4.5*cm])
    ...
    story.append(Paragraph("ITEM RESPONSES", section_s))
    ...
    for i in sorted(answers.keys()):
        ...
        rows.append([
            ...
            Paragraph(f"<b>{sc}</b>", ParagraphStyle("SC", fontName="Helvetica-Bold", fontSize=9,
                      textColor=RED if sc >= 2 else DARK, alignment=TA_CENTER)),
        ])
    row_styles = [
    ...
    for row_idx, i in enumerate(sorted(answers.keys()), start=1):
        if answers[i]["score"] >= 2:
            row_styles.append(("BACKGROUND",(0,row_idx),(-1,row_idx),colors.HexColor("#FFF3F3")))
    it = Table(rows, colWidths=[1*cm, 3.5*cm, 11*cm, 1.5*cm])
    it.setStyle(TableStyle(row_styles))
    story.append(it)
    story.append(Spacer(1, 0.5*cm))
    ...
    for line in report_text.split("\n"):
        ...
        elif line.startswith("⚠") or "suicidal" in line.lower():
...
def send_report_email(pdf_path, client_name, total_score, severity, filename):
    ...
    <html><body style="font-family:Georgia,serif;color:#1C1917;background:#F7F3EE;padding:24px;">
      <div style="max-width:560px;margin:0 auto;background:white;border:1px solid #DDD5C8;border-radius:4px;padding:32px;">
        ...
        <p style="color:#8B7355;font-size:12px;letter-spacing:0.08em;text-transform:uppercase;margin-top:0;">New Assessment Submitted</p>
...
.assessment-header h1 { font-size:2.6rem; font-weight:300; letter-spacing:0.04em; margin-bottom:0.4rem; }
.assessment-header p { color:var(--warm); font-size:0.9rem; font-weight:300; letter-spacing:0.08em; text-transform:uppercase; }
div[data-testid="stRadio"] > label { display:none; }
div[data-testid="stRadio"] > div { gap:0.5rem !important; }
div[data-testid="stRadio"] > div > label { background:var(--cream) !important; border:1px solid var(--border) !important; border-radius:3px !important; padding:0.65rem 1rem !important; cursor:pointer !important; transition:all 0.15s ease !important; font-size:0.88rem !important; color:var(--deep) !important; font-family:'Jost',sans-serif !important; width:100% !important; }
...
.progress-bar-fill { height:3px; border-radius:2px; background:linear-gradient(90deg,var(--warm),var(--accent)); transition:width 0.4s ease; }
.stButton > button { background:var(--selected) !important; color:var(--cream) !important; border:none !important; padding:0.85rem 3rem !important; font-family:'Jost',sans-serif !important; font-size:0.85rem !important; font-weight:400 !important; letter-spacing:0.12em !important; text-transform:uppercase !important; border-radius:2px !important; }
...
.thank-you-block p { color:var(--warm); font-size:0.95rem; font-weight:300; max-width:380px; margin:0 auto; line-height:1.7; }
.warning-box { background:#FFF8F0; border-left:3px solid #E07B39; padding:1rem 1.2rem; border-radius:0 4px 4px 0; font-size:0.88rem; color:#7A3D1A; margin:1rem 0; }
...
if page == "admin":
    ...
    if st.session_state.submitted:
        ...
        st.markdown("""
        <p style="font-size:0.88rem;color:#8B7355;text-align:center;margin-bottom:2rem;font-weight:300;line-height:1.7;">
        ...
        for i, q in enumerate(BDI_QUESTIONS):
            st.markdown(f"""
            <div style="background:white;border:1px solid #DDD5C8;border-radius:4px;padding:1.8rem 2rem;margin-bottom:1.2rem;">
                <div style="font-size:0.72rem;font-weight:500;letter-spacing:0.12em;text-transform:uppercase;color:#C4956A;margin-bottom:0.5rem;">
                    Question {i+1} of {len(BDI_QUESTIONS)}
                </div>
                <div style="font-family:'Cormorant Garamond',serif;font-size:1.15rem;color:#1C1917;margin-bottom:1.2rem;line-height:1.5;">
                    {q['theme']}
                </div>
            </div>""", unsafe_allow_html=True)
        ...
        st.markdown(f"""
        <div style="text-align:center;margin:1.5rem 0 0.5rem 0;font-size:0.78rem;color:#8B7355;letter-spacing:0.08em;">
        ...
        if not all_answered and answered_count > 0:
            st.markdown("""
            <div class="warning-box">⚠ Please answer all questions before submitting.</div>
            """, unsafe_allow_html=True)
